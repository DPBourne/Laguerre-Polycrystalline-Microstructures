...
for iEdge = 1:length(edgeCrossInds)
    ...
    % indFaces = find(sum(ismember(f, edge), 2) == 2); This line generates
    % an error in our version of MATLAB. The following lines fix this error
    indFaces=[];
    for ii=1:length(f)
        if(sum(ismember(f{ii}, edge), 2) == 2)
            indFaces=[indFaces;ii];
        end
    end
